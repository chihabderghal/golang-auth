...
// TestFetchAllAdminsByAdmin tests the endpoint for fetching all users when accessed by an admin user.
// It sets up a test database, seeds fake users, logs in an admin user, and verifies that the
// response from the endpoint is a success (HTTP 200).
func TestFetchAllAdminUsersByAdmin(t *testing.T) {
	...
	req2 := httptest.NewRequest("GET", "/api/users/get-admin-users", nil)
	...
	// Verify that the response status code is 200 OK for successful login
	assert.Equal(t, http.StatusOK, resp2.StatusCode, "Expected status code 200 OK when getting all users")
}

func TestFetchAdminByNormalUser(t *testing.T) {
	// Set up the test application and create a test database
	app := tests_setup.Setup()
